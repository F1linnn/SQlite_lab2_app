...
class Technologist:
    ...
    def show_devices():
        with DataBase(DB) as connection:
            ...
            cur.execute("""SELECT * FROM `техника`""")
            return cur.fetchall()
    ...
    def show_client():
        with DataBase(DB) as connection:
            cur = connection.cursor()
            cur.execute("""SELECT * FROM `список_сотрудников` ORDER BY IDсотрудника """)
            return cur.fetchall()
    ...
    def add_client(*args):
        with DataBase(DB) as connection:
            cur = connection.cursor()
            cur.execute("""INSERT INTO `список_сотрудников` VALUES (?,?,?,?,?,?,?,?)""", tuple(args))
        print('Сотрудник успешно добавлен!')

    @staticmethod
    def del_client(category,key):
        categories = ['IDсотрудника', 'ФИО', 'Должность']
        with DataBase(DB) as connection:
            cur = connection.cursor()
            cur.execute(f"DELETE FROM `список_сотрудников` WHERE {categories[category]} = '{key}'")
    ...
    def add_device(*args):
        with DataBase(DB) as connection:
            cur = connection.cursor()
            cur.execute("""INSERT INTO `техника` VALUES (?,?,?,?,?,?)""", tuple(args))
        print('Техника добавлена добавлен!')

    @staticmethod
    def del_device(category, key):
        categories = ['IDтехники']
        with DataBase(DB) as connection:
            cur = connection.cursor()
            cur.execute(f"DELETE FROM `техника` WHERE {categories[category]} = '{key}'")
        print('Техника списана!')
